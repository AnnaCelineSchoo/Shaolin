...
function StrengthTable({ type, week, reps, sets, amount, holdingTime }) {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.PUBLIC_URL}/workouts/strength.json`
        );
        ...
        const excersisesList = excersiseNumbers.map(
          (index) => workoutData[index]
        );
  ...
  return (
    <div>
      <h4 className="bg-black text-white text-center py-2">
        {type.replaceAll("_", " ")}
      </h4>
      <div className="table-responsive">
        <table className="table table-striped">
          <thead>
            <tr>
              <th>Excersise</th>
              {type.includes("forms") || type.includes("basics") ? null : (
                <th>{type === "stances" ? "Holding time (sec)" : "reps"}</th>
              )}
              {type.includes("forms") || type.includes("basics") ? null : (
                <th>Sets</th>
              )}
            ...
            {excersises.map((excersise, index) => {
              return (
                <tr key={index}>
                  ...
                  {type.includes("forms") || type.includes("basics") ? null : (
                    <td>{type === "stances" ? holdingTime : reps}</td>
                  )}
                  {type.includes("forms") || type.includes("basics") ? null : (
                    <td>{sets}</td>
                  )}
                  ...
                  {!excersise.embedLink || excersise.embedLink === "" ? (
                    ...
                    <td className="no-print">
                      <Link
                        to={{
                          pathname: `/video/${
                            excersise.embedLink.split("?")[0]
                          }`, // Correctly extract video ID from the embedLink
                          search: `?description=${encodeURIComponent(
                            excersise.description
                          )}&name=${encodeURIComponent(excersise.exercise)}`, // Pass description and name as query parameters
                        }}
                        style={{ textDecoration: "none", color: "inherit" }} // Optional styling for cleaner appearance
                      >
                        <IconButton className="link-danger link-offset-2 link-offset-3-hover link-underline link-underline-opacity-0 link-underline-opacity-75-hover">
                          <YouTubeIcon />
                        </IconButton>
                      </Link>
                    </td>
